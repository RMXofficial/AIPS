...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> tmp1=firstSorcererCards.getFirst();
        int power1=0;
        SLLNode<Card> strongest1=firstSorcererCards.getFirst();
        while (tmp1 != null){
            if (tmp1.element.significance()>power1){
                power1=tmp1.element.significance();
                strongest1=tmp1;
            }
            tmp1=tmp1.succ;
        }
        firstSorcererCards.delete(strongest1);
        SLLNode<Card> tmp2=secondSorcererCards.getFirst();
            for (int i = 0; i <secondSorcererCards.size()/2; i++) {
                tmp2=tmp2.succ;
            }
        secondSorcererCards.insertBefore(strongest1.element,tmp2);
    }
