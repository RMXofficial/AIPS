...
public class Glavna {
    public static void work(SLL<Task> toDo, SLL<Task> inProgress) {
        ...
        SLLNode<Task> tmp3 = toDo.getFirst();
        SLLNode<Task> tmp4 = inProgress.getFirst();
        int min = 100;
        int max = 0;
        while (tmp1 != null) {
            if(max<(2*tmp1.element.getHours()*tmp1.element.getPriority())){
                max=2*tmp1.element.getHours()*tmp1.element.getPriority();
                tmp3=tmp1;
            }
            tmp1 = tmp1.succ;
        }
        while (tmp2 != null) {
            if(min>(2*tmp2.element.getHours()*tmp2.element.getPriority())){
                min=2*tmp2.element.getHours()*tmp2.element.getPriority();
                tmp4=tmp2;
            }
            tmp2 = tmp2.succ;
        }
        inProgress.insertFirst(tmp3.element);
        toDo.delete(tmp3);
        toDo.insertLast(tmp4.element);
        inProgress.delete(tmp4);
    }
